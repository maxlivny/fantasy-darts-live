...
def parse_tables(html: str, players: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for table in soup.select("table"):
        # Берём только таблицы расписания матчей.
        # Турнирная сетка Wikipedia использует rowspan/colspan, из-за чего
        # значение w/o может ошибочно соединиться с игроками из соседнего матча.
...
def filter_completed_matches(
    matches: list[dict[str, Any]],
    settings: dict[str, Any],
) -> list[dict[str, Any]]:
    """
    Wikipedia во время матча показывает текущий счёт (например, 4-1).
    Такой счёт нельзя считать завершённым.

    Матч принимается только если:
    - это техническая победа w/o; или
    - победитель достиг требуемого количества легов для данного раунда.
    """
    raw_legs = settings.get("winning_legs_by_round", {})
    winning_legs: dict[int, int] = {}

    if isinstance(raw_legs, dict):
        for key, value in raw_legs.items():
            try:
                winning_legs[int(key)] = int(value)
    ...
    for match in matches:
        ...
        try:
            round_no = int(match.get("round"))
        except (TypeError, ValueError):
            round_no = None

        required_legs = winning_legs.get(round_no) if round_no is not None else None

        # Если требования по раундам настроены, неизвестный раунд не принимаем:
        # иначе промежуточный счёт снова может попасть в завершённые матчи.
        if winning_legs:
            if required_legs is None or winner_score < required_legs:
                continue
...
def main() -> int:
    ...
    matches = filter_completed_matches(matches, project.get("settings", {}))
