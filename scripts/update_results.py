...
def calculate(project: dict[str, Any], matches: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # Если на странице уже есть более поздний раунд, игрок, который дошёл
    # только до более ранней стадии и не отмечен явным проигравшим из-за
    # отсутствующей строки DartConnect, всё равно уже не может оставаться
    # в турнире. Например, если полуфиналы и финал сыграны, участник с
    # достигнутой стадией 5 (четвертьфинал) должен считаться выбывшим.
    latest_round = max(
        (int(match.get("round", 1)) for match in matches),
        default=0,
    )
    inferred_eliminated: set[str] = set()
    if latest_round > 0:
        for player, stage in reached_stage.items():
            if stage <= latest_round:
                inferred_eliminated.add(player)
    eliminated.update(inferred_eliminated)
