...
def parse_schedule_wikitext(
    ...
    for row_match in row_pattern.finditer(wikitext):
        ...
        if cells and re.match(r"rowspan\s*=", cells[0], flags=re.I):
            cells = cells[1:]

        if len(cells) < 3:
            continue

        player1_text = clean_wikitext_cell(cells[0])
        score_text = clean_wikitext_cell(cells[1])
        player2_text = clean_wikitext_cell(cells[2])

        player1 = resolve_player(player1_text, players)
        player2 = resolve_player(player2_text, players)
