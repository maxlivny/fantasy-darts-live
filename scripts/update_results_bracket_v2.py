...
def parse_bracket_wikitext(
    ...
    pattern = re.compile(
        r"^\s*\|\s*(RD\d+\s*-\s*(?:team|score)\s*\d+)\s*=\s*(.*?)\s*$",
        flags=re.I | re.M,
    )

    for key, value in pattern.findall(wikitext):
    ...
    rounds = sorted(
        {
            int(match.group(1))
            for key in params
            if (match := re.fullmatch(r"RD(\d+)-TEAM\d+", key))
        }
    )
    ...
    for round_no in rounds:
        indices = sorted(
            int(match.group(1))
            for key in params
            if (match := re.fullmatch(rf"RD{round_no}-TEAM(\d+)", key))
        )
        if not indices:
            continue
        ...
        for index in range(1, max_index + 1, 2):
            suffix1 = f"{index:02d}"
            suffix2 = f"{index + 1:02d}"

            raw_player1 = params.get(f"RD{round_no}-TEAM{suffix1}", "")
            raw_player2 = params.get(f"RD{round_no}-TEAM{suffix2}", "")
            raw_score1 = params.get(f"RD{round_no}-SCORE{suffix1}", "")
            raw_score2 = params.get(f"RD{round_no}-SCORE{suffix2}", "")

            player1 = resolve_player(clean_wikitext_value(raw_player1), players)
            player2 = resolve_player(clean_wikitext_value(raw_player2), players)

            if not player1 or not player2 or player1 == player2:
                continue

            score_text1 = clean_wikitext_value(raw_score1)
            score_text2 = clean_wikitext_value(raw_score2)

            walkover1 = bool(WALKOVER_RE.search(score_text1))
            walkover2 = bool(WALKOVER_RE.search(score_text2))

            if walkover1 ^ walkover2:
                winner = player1 if walkover1 else player2
                loser = player2 if walkover1 else player1
                display_score = "w/o"
            else:
                score_match1 = re.search(r"(?<!\d)(\d{1,2})(?!\d)", score_text1)
                score_match2 = re.search(r"(?<!\d)(\d{1,2})(?!\d)", score_text2)

                if not (score_match1 and score_match2):
                    continue
                ...
                if not is_completed_numeric_score(score1, score2, required_legs):
                    continue
...
def parse_schedule_tables(
    ...
    for table in soup.select("table"):
        ...
        for row in table.select("tr"):
            ...
            player1 = resolve_player(texts[player1_index], players)
            player2 = resolve_player(texts[player2_index], players)
...
def fetch_matches(
    ...
    api_response = requests.get(
        ...
        timeout=30,
        headers={"User-Agent": "FantasyDartsLive/2.0 (public standings updater)"},
    )
    api_response.raise_for_status()
    payload = api_response.json().get("parse", {})

    wikitext = payload.get("wikitext", "")
    rendered_html = payload.get("text", "")

    bracket_matches = parse_bracket_wikitext(
        wikitext,
        players,
