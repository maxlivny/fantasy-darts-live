...
def print_wikipedia_diagnostics(wikitext: str) -> None:
    """Печатает в GitHub Actions структуру wiki-разметки, не засоряя лог всей страницей."""
    source = str(wikitext or "")
    print("=== WIKIPEDIA DIAGNOSTICS START ===", file=sys.stderr)
    print(f"Wikitext chars: {len(source)}", file=sys.stderr)

    # Какие шаблоны реально используются на странице.
    templates = re.findall(r"\{\{\s*([^|{}\n]+)", source)
    template_counts: dict[str, int] = {}
    for name in templates:
        cleaned = re.sub(r"\s+", " ", name).strip()
        if cleaned:
            template_counts[cleaned] = template_counts.get(cleaned, 0) + 1
    common = sorted(template_counts.items(), key=lambda x: (-x[1], x[0]))[:40]
    print("Templates:", common, file=sys.stderr)

    # Ищем строки, похожие на сетку/результаты/раунды.
    interesting_re = re.compile(
        r"(schedule|draw|bracket|round|first round|second round|"
        r"third round|quarter|semi|final|last 32|last 16|"
        r"RD\d+|6\s*[-–—:]\s*\d|7\s*[-–—:]\s*\d|8\s*[-–—:]\s*\d)",
        re.I,
    )
    interesting = []
    for lineno, raw in enumerate(source.splitlines(), start=1):
        line = raw.strip()
        if line and interesting_re.search(line):
            interesting.append((lineno, line[:500]))

    print(f"Interesting lines found: {len(interesting)}", file=sys.stderr)
    for lineno, line in interesting[:220]:
        print(f"WIKI L{lineno}: {line}", file=sys.stderr)

    print("=== WIKIPEDIA DIAGNOSTICS END ===", file=sys.stderr)
...
def fetch_wikipedia_matches(
    ...
    raise RuntimeError(
        ...
        "Диагностическая структура страницы напечатана выше между "
        "WIKIPEDIA DIAGNOSTICS START/END."
    )
...
def calculate(project: dict[str, Any], matches: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # позднего раунда: в текущем раунде остальные пары могут ещё не сыграть.
