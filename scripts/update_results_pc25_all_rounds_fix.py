...
def dartconnect_round_from_heading(value: str, rounds_count: int) -> int | None:
    """Определяет раунд по заголовку секции DartConnect.

    Поддерживает дополнительные подписи вроде
    ``Final (Select Match Score to Review)`` и варианты с пробелами/дефисами.
    """
    text = normalize_name(value)
    if not text:
        return None

    if re.search(r"\bsemi[\s-]*final", text):
        return max(1, rounds_count - 1)
    if re.search(r"\bquarter[\s-]*final|\blast\s*8\b", text):
        return max(1, rounds_count - 2)
    if re.search(r"\b(?:grand\s+)?finals?\b", text):
        return rounds_count

    match = re.search(r"\b(?:top|last|round\s+of)\s*(128|64|32|16|8|4|2)\b", text)
    if match:
        ...
        mapping = {
            ...
            32: 3,
            16: 4,
            8: max(1, rounds_count - 2),
            4: max(1, rounds_count - 1),
            2: rounds_count,
        }
        return min(rounds_count, mapping[field])
...
def parse_dartconnect_dom_rows(
    ...
    for row_index, row in enumerate(rows, start=1):
        ...
        winner_score = max(score1, score2)
        if winner_score >= winning_legs_by_round.get(rounds_count, 10**9):
            round_no = rounds_count
        elif rounds_count >= 2 and winner_score >= winning_legs_by_round.get(rounds_count - 1, 10**9):
...
def fetch_dartconnect_matches(
        ...
        dom_rows = page.evaluate(
            r"""() => {
                ...
                for (const el of document.querySelectorAll('[recap-url][tournament-id]')) {
                    if (scoreRe.test(compactText(el))) candidateSet.add(el);
                }

                const playerSpans = Array.from(
                    document.querySelectorAll('span.truncate.leading-tight')
                );
                for (const span of playerSpans) {
                    let node = span;
                    for (let depth = 0; node && depth < 8; depth++, node = node.parentElement) {
                        const names = node.querySelectorAll
                            ? node.querySelectorAll('span.truncate.leading-tight')
                            : [];
                        if (names.length >= 2 && scoreRe.test(compactText(node))) {
                ...
                const candidates = Array.from(candidateSet).filter(el =>
                    !Array.from(candidateSet).some(other =>
                        other !== el && el.contains(other)
                    )
                );

                const headingCandidates = Array.from(document.querySelectorAll('body *'))
                    .filter(el => {
                        const t = compactText(el);
                        return t.length > 0 && t.length <= 120 && headingRe.test(t);
                    });
                ...
                for (const el of candidates) {
                    ...
                    let score = Array.from(el.querySelectorAll('span'))
                        .map(sp => compactText(sp))
    ...
    for match in matches:
        parsed_round = int(match.get("round", 0))
